    ...
    , ui(new Ui::MainWindow)
{
    ...
    timergenerarenemigos = new QTimer;
    timerenemigosmultijugador = new QTimer;
    ...
    triturado = new QMovie(":/gif/triturado.gif"); //darle el gif al QMovie
    ui->Triturado->setMovie(triturado); //darle al label elQMovie

    portal = new QMovie(":/gif/portal.gif"); //darle el Gif al QMovie
    ui->Portal->setMovie(portal); //darle al label el QMovie
...
void MainWindow::mover_enemigosmultijugador()
{
    if(vidas->getVidas()==2)
    {
        for(int i=0; i<enemigos.size();i++) //ciclo que lee toda la lista de enemigos
        {
            ...
            if(enemigos.at(i)->getPosy()<-750 && enemigos.at(i)->isActive())
            {
                enemigos.at(i)->hide();
            }
            choque_multijugador(enemigos.at(i));
        }
    }

    if(vidas->getVidas()==1)
    {
        for(int i=0; i<enemigos2.size();i++) //ciclo que lee toda la lista de enemigos
        {
            if(enemigos2.at(i)->isVisible())
            {
                enemigos2.at(i)->actualizarposicion_derecha(); //actualiza la posicion
                enemigos2.at(i)->actualizarvelocidad(); //actualiza la velocidad
                enemigos2.at(i)->setX(enemigos2.at(i)->getPosx()); //setea la posicion en x
                enemigos2.at(i)->setY(-enemigos2.at(i)->getPosy());//setea la posicion en y
            }

            if(enemigos2.at(i)->getPosy()<-750 && enemigos2.at(i)->isActive())
            {
                enemigos2.at(i)->hide();
            }
            choque_multijugador(enemigos2.at(i));
        }
    }
}
...
void MainWindow::nivel3()
{
    ...
    if(!pastilla->isActive())
    {
        escena->addItem(pastilla);
    }
    pastilla->show();
...
void MainWindow::nivel_multijugador()
{
    ...
    juego = true; //se puede jugar
    musica->stop();
    musica->setMedia(QUrl("qrc:/musica/musica_nivel3.mp3")); //le doy la cancion correspondiente
    ui->graphicsView->setBackgroundBrush(QPixmap(":/imagenes/fondo_nivel3.jpg"));
    puntos->setPuntaje(0);
    puntos->tiempo(0);
    chillin->setVelocidad(10);
    ...
    timerenemigo2->stop();
    timerenemigo3->stop();
    chillin->setPosx(475); //actualizar posicion
    musica->play();
    ...
    connect(timergenerarenemigos,SIGNAL(timeout()),this,SLOT(generar_enemigos()));
    connect(timerenemigosmultijugador,SIGNAL(timeout()),this,SLOT(mover_enemigosmultijugador()));
...
void MainWindow::keyPressEvent(QKeyEvent *event)
{
    ...
    {
        //tecla D
        bool D=((event->key()==Qt::Key_D && timerenemigo->isActive()) || (event->key()==Qt::Key_D && timerenemigo2->isActive()) || ((event->key()==Qt::Key_D && timerenemigo3->isActive()) || ((event->key()==Qt::Key_D && timerenemigosmultijugador ->isActive())) ));
        ...
        //tecla A
        bool A=((event->key()==Qt::Key_A && timerenemigo->isActive()) || (event->key()==Qt::Key_A && timerenemigo2->isActive()) || (event->key()==Qt::Key_A && timerenemigo3->isActive()) || (event->key()==Qt::Key_A && timerenemigosmultijugador->isActive()) );
        ...
        //tecla W
        bool W=((event->key()==Qt::Key_W && timerenemigo->isActive()) || (event->key()==Qt::Key_W && timerenemigo2->isActive()) ||(event->key()==Qt::Key_W && timerenemigo3->isActive()) || (event->key()==Qt::Key_W && timerenemigosmultijugador->isActive()) );
        ...
        if(event->key()==Qt::Key_Space) //condicion que pregunta si la tecla que se oprimio fue shift
        {
            ...
            if(multijugador)
            {
                timergenerarenemigos->start(333);
                timerenemigosmultijugador->start(15);
            }
        }
    }
}
...
void MainWindow::choque_multijugador(enemigo *bola)
{
    if(chillin->collidesWithItem(bola)&&bola->isVisible()) //condicion que pregunta si hubo colision con una bola amistosa
    {
        ...
        timergenerarenemigos->stop();
        timerenemigosmultijugador->stop();
        vidas->disminuir_salud(); //disminuye las vidas
        if(vidas->getVidas()==1) //condicion que pregunta si las vidas son diferentes de 0
        {
            chillin->setImagen(3);
            ...
            corte->play();
            triturado->start();
            chillin->setPosx(475); //resetea la posicion de la chillin
            probabilidad->setPos(chillin->getPosx()+25,chillin->getPosy()-50); //da la posision de la probabilidad siempre al lado de el personaje principal
        }
    ...
    if(pastilla->collidesWithItem(chillin))
    {
        if(vidas->getVidas()==2)
        {
            probabilidad->setTope(100);
            if(chillin->getPosx()>900)
            {
                pastilla->setPosx(50);
            }

            if(chillin->getPosx()<100)
            {
                pastilla->setPosx(950);
            }
...
void MainWindow::actualizar_puntaje_multijugador(int p)
{
    ...
    {
        p=0;
        for(int i=0;i<enemigos2.size();i++) //ciclo que recorre la lista de los enemigos
        {
            if(!enemigos2.at(i)->isVisible()) //condicion que pregunta si el enemigo no es visible
            {
                p=p+1; //suma 10 a la variable ps
            }
        }
        ...
        if(vidas->getVidas()==1) //condicion que pregunta si las vidas son diferentes de 0
        {
            ...
            if(multijugador==true)
            {
                ...
                for(int i=0;i<enemigos.size();i++) //ciclo que recorre toda la lista de enemigos
                {
                    ...
                    {
                        enemigos.at(i)->hide(); //remueve los enemigos visibles
                    }
                }
            }
        }
...
void MainWindow::game_over_multijugador()
{
    ...
    musica->stop();
    escena->removeItem(chillin); //elimina el personaje principal
    for(int i=0;i<enemigos2.size();i++) //ciclo que recorre toda la lista de enemigos
    {
        if(enemigos2.at(i)->isVisible()) //condicion que pregunta si un enemigo es visible
        {
            escena->removeItem(enemigos2.at(i)); //remueve los enemigos visibles
        }
    }
    escena->removeItem(puntos); //remueve el puntaje
    escena->removeItem(probabilidad); //remueve la probabilidad
...
void MainWindow::on_boton_volveralmenu_clicked()
{
    ...
    delete timergenerarenemigos;
    delete timerenemigosmultijugador;
...
void MainWindow::generar_enemigos()
{
    ...
    if(vidas->getVidas()==2)
    {
        ...
        enemigos.push_back(new enemigo(x,10,largo,ancho,0,0)); //creo enemigo 1); //agrego enemigo 1 a la lista de enemigos
        enemigos.back()->setImagen(imagen);
        enemigos.back()->setScale(escala);
        if(imagen==4)
        {
            enemigos.back()->setColumnas_fila(0,82);
        }
        if(imagen==1)
        {
            enemigos.back()->setColumnas_fila(0,82);
    ...
    if(vidas->getVidas()==1)
    {
        ...
        enemigos2.push_back(new enemigo(x,10,largo,ancho,0,0)); //creo enemigo 1); //agrego enemigo 1 a la lista de enemigos
        enemigos2.back()->setImagen(imagen);
        enemigos2.back()->setScale(escala);
        if(imagen==4)
        {
            enemigos2.back()->setColumnas_fila(0,82);
        }
        if(imagen==1)
        {
            enemigos2.back()->setColumnas_fila(0,82);
        }
        escena->addItem(enemigos2.back());
    }
...
{
    ui->graphicsView->setBackgroundBrush(QPixmap(":/imagenes/tutorial.jpg"));
